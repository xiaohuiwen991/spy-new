...
importing('socket', 'dict', function(){
    ...
    $('#user-name').on('click',function () {
        var editPwdWin = $open('#edit-pwd-block',{width:416,height:320,top:180,title:'&nbsp系统用户修改个人密码',closable:false});
    });
    ...
    $('#save-edit-pwd').on('click',function () {
        ...
        if($('#confirm-new-pwd').val() != newPwd){
            toast(' 请确认两次输入的新密码是否一致！').width(300).warn();
            return false;
        }
    ...
    //默认登录用户管理
    mainFrame.src=getViewPath('database-connection.html');
